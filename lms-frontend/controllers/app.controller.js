...
const App = {
  ...
  initLandingNav() {
    ...
    nav.querySelectorAll(".nav-link, .nav-links .btn").forEach((link) => {
  ...
  init() {
    ...
    this.sidebarCollapsed =
      savedSidebarState === true || savedSidebarState === "true";
    ...
    const observer = new MutationObserver(() => {
      document.body.style.overflow = _sb.classList.contains("mobile-open")
        ? "hidden"
        : "";
    });
  ...
  applyProfileImage(
    user = DashboardController.currentUser || Storage.get("ijla_session"),
  ) {
    const image =
      user?.avatar_url ||
      (user?.id ? Storage.get(`ijed_profile_image_${user.id}`) : null);
    [
