...
const SUPABASE_ANON_KEY =
  "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InFqa29xem52cnZsc3phY3ZtZXVnIiwicm9sZSI6ImFub24iLCJpYXQiOjE3ODI1NjQ2OTAsImV4cCI6MjA5ODE0MDY5MH0.hHRQpLeZyBKeIskeFIBshbv-e_jJkw07lNktTvPSSKE";
...
class LMSAdminAPI {
  ...
  async getUsers(params = {}) {
    let q = this.sb
      .from("users")
      .select(
        "id, email, first_name, last_name, role_id, is_active, created_at, roles(name)",
      );
  ...
  async updateUser(id, fields) {
    return this._throwIfError(
      await this.sb.from("users").update(fields).eq("id", id).select().single(),
    );
  }
  ...
  async createTeacherProfile({
    ...
    const result = this._throwIfError(
      await this.sb
        .from("teachers")
        .insert({ user_id, employee_id, specialization, contact_number })
        .select()
        .single(),
    );
  ...
  async assignTeacherToClass({ teacher_id, class_id, subject_id, schedule }) {
    const result = this._throwIfError(
      await this.sb
        .from("teacher_class_assignments")
        .insert({ teacher_id, class_id, subject_id, schedule })
        .select()
        .single(),
    );
    this.clearCache("teachers");
  ...
  async getStudents() {
    return this._cached("students:all", 60_000, async () => {
      ...
      const sectionAssignments = studentIds.length
        ? this._throwIfError(
            await this.sb
  ...
  async enrollStudentSubjects(studentId, subjectIds) {
    const result = this._throwIfError(
      await this.sb.rpc("enroll_student_subjects", {
  ...
  async unenrollStudentSubject(studentId, subjectId) {
    const result = this._throwIfError(
      await this.sb.rpc("unenroll_student_subject", {
        p_student_id: studentId,
  ...
  async getClasses() {
    return this._cached("classes:all", 60_000, async () =>
      this._throwIfError(
        await this.sb.from("classes").select("*").eq("is_active", true),
      ),
    );
  }
  ...
  async createClass({ name, grade_level, school_year }) {
    const result = this._throwIfError(
      await this.sb
        .from("classes")
        .insert({ name, grade_level, school_year })
        .select()
        .single(),
    );
  ...
  async getSection(id) {
    return this._throwIfError(
      await this.sb.from("sections").select("*").eq("id", id).single(),
    );
  }
  ...
  async deleteSection(id) {
    const result = this._throwIfError(
      await this.sb.from("sections").delete().eq("id", id),
    );
    this.clearCache("sections");
    return result;
  }
  ...
  async getModule(id) {
    return this._throwIfError(
      await this.sb.from("modules").select("*").eq("id", id).single(),
    );
  }
  ...
  async deleteModule(id) {
    const result = this._throwIfError(
      await this.sb.from("modules").delete().eq("id", id),
    );
    this.clearCache("modules");
    return result;
  }
  ...
  async getActivities(module_id) {
    return this._cached(`activities:${module_id}`, 60_000, async () =>
      this._throwIfError(
        await this.sb.from("activities").select("*").eq("module_id", module_id),
      ),
    );
  }
  ...
  async deleteActivity(id) {
    const result = this._throwIfError(
      await this.sb.from("activities").delete().eq("id", id),
    );
    this.clearCache("activities");
    return result;
  }
      ...
      async () => {
        const data = this._throwIfError(
          await this.sb
            .from("student_section_assignments")
            .select(
              "students(*, users(first_name, last_name)), sections!inner(class_id)",
            )
  ...
  async getClassModuleReads(classId, subjectId = null) {
    ...
    return this._cached(cacheKey, 30_000, async () => {
      ...
      const { data: reads } = await this.sb
        .from("student_module_reads")
  ...
  async createTeacherActivity(payload) {
    ...
    const activity = this._throwIfError(
      await this.sb
        .from("activities")
        .insert({ ...activityFields, teacher_id })
        .select()
        .single(),
    );
    for (const [i, q] of (questions || []).entries()) {
      ...
      if (choices?.length) {
        await this.sb
          .from("activity_question_choices")
          .insert(
            choices.map((c, j) => ({
              ...c,
  ...
  async getTeacherActivities({ module_id, subject_id } = {}) {
    return this._cached(
      `teacher:activities:${module_id || ""}:${subject_id || ""}`,
      60_000,
      async () => {
        let q = this.sb
          .from("activities")
    ...
    if (questions) {
      ...
      for (const [i, q] of questions.entries()) {
        ...
        const question = this._throwIfError(
          await this.sb
            .from("activity_questions")
            .insert({ ...qFields, activity_id: id, order: q.order ?? i })
            .select()
            .single(),
        );
        if (choices?.length) {
  ...
  async getStudentSubjects(semesterFilter = null) {
    ...
    return this._cached(cacheKey, 60_000, async () => {
      ...
      if (semesterFilter)
        return rows.filter((r) => r.semester === semesterFilter);
      return rows;
    });
  }
  ...
  async getStudentCurrentSemester() {
    ...
    const semesters = [
      ...new Set(all.map((r) => r.semester).filter(Boolean)),
    ].sort();
  ...
  async getStudentActivity(id) {
    ...
    const questions = (rawQuestions || []).map((q) => ({
      ...q,
      choices: (q.activity_question_choices || []).sort(
        (a, b) => a.order - b.order,
      ),
    }));
    const studentId = await this._myStudentId();
    ...
    const now = new Date();
    const isPastDue = !!(
      activity.due_date && now > new Date(activity.due_date)
    );
    const isStarted =
      !activity.start_date || now >= new Date(activity.start_date);
    const canAnswer =
      !existingSub && !isPastDue && isStarted && activity.is_published;
    return {
      ...activity,
      questions,
      can_answer: canAnswer,
  ...
  async submitActivityAnswers(activityId, answers) {
    const result = this._throwIfError(
      await this.sb.rpc("submit_activity", {
        p_activity_id: activityId,
        p_answers: answers,
      }),
    );
  ...
  async markModuleRead(moduleId) {
    const result = this._throwIfError(
      await this.sb.rpc("mark_module_read", { p_module_id: moduleId }),
    );
    this.clearCache("student:dashboard");
    return result;
  }
  ...
  async sendAnnouncement(title, message, target = "all") {
    const { data, error } = await this.sb.rpc("send_announcement", {
      p_title: title,
      p_message: message,
      p_target: target,
    });
    if (error) throw new Error(error.message);
    // The RPC returns the list of user IDs that received the notification
    return {
      message: `Announcement sent to ${data?.length || 0} user(s).`,
      sent_to: data?.length || 0,
    };
  }
  async sendAnnouncement(title, message, target = "all") {
    let roleFilter = null;
    ...
    const { data: users } = await this.sb
      ...
      .eq("is_active", true);
    const targets = (users || [])
      .filter((u) => !roleFilter || u.roles.name === roleFilter)
      .map((u) => u.id);
    ...
    return {
      message: `Announcement sent to ${rows.length} user(s).`,
      sent_to: rows.length,
    };
  }
  ...
  async getAttendanceSections() {
    ...
    const data = this._throwIfError(
      await this.sb
        .from("teacher_class_assignments")
        .select(
          "*, subjects(id, name), classes(id, name, grade_level, school_year, sections(id, name))",
        )
  ...
  async getAttendanceSectionStudents(
    classId,
    { subjectId = null, term = null } = {},
  ) {
    const { data, error } = await this.sb.rpc(
  ...
  async getAttendanceSession(sessionId) {
    return this._throwIfError(
      await this.sb
        ...
        .select(
          "*, attendance_records(*, students(*, users(first_name, last_name)))",
        )
        .eq("id", sessionId)
        .single(),
    );
  ...
  async deleteAttendanceSession(sessionId) {
    return this._throwIfError(
      await this.sb.from("attendance_sessions").delete().eq("id", sessionId),
    );
  }
  ...
  async getBayesianAnalytics(targetGrade = 90, subjectId = null) {
    ...
    const [
      ...
      AnalyticsEngine.getImprovementProbability(
        this.sb,
        studentId,
        targetGrade,
        subjectId,
  ...
  async getPredictedGrade(subjectId = null) {
    return AnalyticsEngine.getPredictedFinalGrade(
      this.sb,
      await this._myStudentId(),
      subjectId,
    );
  }

  async getImprovementProbability(targetGrade = 90, subjectId = null) {
    return AnalyticsEngine.getImprovementProbability(
      this.sb,
      await this._myStudentId(),
      targetGrade,
      subjectId,
  ...
  async getRiskAssessment() {
    return AnalyticsEngine.getRiskAssessment(
      this.sb,
      await this._myStudentId(),
    );
  }
